...
Yellow is the original crab recolored. Red hats use original crabby.
```

## export-naming.md

```md
# export-naming.md — v.102

Current product version: **v.101** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.102**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.100",
    "file": "crabby-beach-v.100-code.md",
    "bytes": 182476,
    "mtime": "2026-09-10T14:55:53.200Z"
  },
  "codebase": {
    "version": "v.090",
...
async function loadAssets(): Promise<Assets> {
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) => [
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=101`)),
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-1.png?v=101`)),
      ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${i}.png?v=101`))),
    ]),
  );
...
function lookSrc(color: CrabColor, hat: CrabHat) {
  ...
  return assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=101`);
}
