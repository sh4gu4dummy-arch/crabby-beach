...
Green magenta halo removed; brush kept.
```

## export-naming.md

```md
# export-naming.md — v.107

Current product version: **v.106** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.107**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.105",
    "file": "crabby-beach-v.105-code.md",
    "bytes": 182693,
    "mtime": "2026-09-10T16:10:59.956Z"
  },
  "codebase": {
    "version": "v.090",
...
export const APP_VERSION = "v.106";
...
async function loadAssets(): Promise<Assets> {
  ...
  const skinFiles = skinIds.flatMap((id) => [
    loadImage(assetUrl(`game/crabby/${id}/idle-0.png?v=106`)),
    loadImage(assetUrl(`game/crabby/${id}/idle-1.png?v=106`)),
    ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/${id}/walk-${i}.png?v=106`))),
  ]);
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) => [
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=106`)),
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-1.png?v=106`)),
      ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${i}.png?v=106`))),
    ]),
  );
