...
Green crab rebuilt on beige so the magenta halo is gone.
```

## export-naming.md

```md
# export-naming.md — v.109

Current product version: **v.108** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.109**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.107",
    "file": "crabby-beach-v.107-code.md",
    "bytes": 183695,
    "mtime": "2026-09-10T16:49:32.852Z"
  },
  "codebase": {
    "version": "v.090",
...
function skinSrc(color: CrabColor) {
  ...
  if (color === "green") return assetUrl("game/crabby/green/idle-0.png?v=108");
...
function lookSrc(color: CrabColor, hat: CrabHat) {
  ...
  return assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=108`);
}
