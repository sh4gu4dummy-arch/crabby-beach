...
Local git + GitHub: every version. Do not ask.
Zip/APK: only when asked. Labels = files on disk.
Publish: user clicks it.
Imagine stills: Method C HTTP bypass (scripts/imagine-image.py). Do not loop the Imagine tool.
```

## export-naming.md

```md
# export-naming.md — v.103

Current product version: **v.102** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.103**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.101",
    "file": "crabby-beach-v.101-code.md",
    "bytes": 182503,
    "mtime": "2026-09-10T15:17:22.920Z"
  },
  "codebase": {
    "version": "v.090",
...
export const APP_VERSION = "v.102";
...
async function loadAssets(): Promise<Assets> {
  ...
  const skinFiles = skinIds.flatMap((id) => [
    loadImage(assetUrl(`game/crabby/${id}/idle-0.png?v=101`)),
    loadImage(assetUrl(`game/crabby/${id}/idle-1.png?v=101`)),
    ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/${id}/walk-${i}.png?v=101`))),
  ]);
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) => [
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=102`)),
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-1.png?v=102`)),
      ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${i}.png?v=102`))),
    ]),
  );
...
function lookSrc(color: CrabColor, hat: CrabHat) {
  ...
  return assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=102`);
}
