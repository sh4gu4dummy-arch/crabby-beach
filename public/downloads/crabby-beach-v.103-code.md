...
Red/yellow hat looks are the original crab, not a second character.
```

## export-naming.md

```md
# export-naming.md — v.104

Current product version: **v.103** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.104**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.102",
    "file": "crabby-beach-v.102-code.md",
    "bytes": 182529,
    "mtime": "2026-09-10T15:29:25.928Z"
  },
  "codebase": {
    "version": "v.090",
...
async function loadAssets(): Promise<Assets> {
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) => [
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=103`)),
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-1.png?v=103`)),
      ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${i}.png?v=103`))),
    ]),
  );
...
function lookSrc(color: CrabColor, hat: CrabHat) {
  ...
  return assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=103`);
}
