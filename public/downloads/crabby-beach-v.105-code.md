...
Walk and paint radius scale up each hour; 12am is +50%.
```

## export-naming.md

```md
# export-naming.md — v.106

Current product version: **v.105** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.106**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.104",
    "file": "crabby-beach-v.104-code.md",
    "bytes": 182520,
    "mtime": "2026-09-10T15:52:03.908Z"
  },
  "codebase": {
    "version": "v.090",
...
export const APP_VERSION = "v.105";
...
async function loadAssets(): Promise<Assets> {
  ...
  const skinFiles = skinIds.flatMap((id) => [
    loadImage(assetUrl(`game/crabby/${id}/idle-0.png?v=104`)),
    loadImage(assetUrl(`game/crabby/${id}/idle-1.png?v=104`)),
    ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/${id}/walk-${i}.png?v=104`))),
  ]);
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) => [
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=104`)),
      loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-1.png?v=104`)),
      ...[1, 2, 3, 4].map((i) => loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${i}.png?v=104`))),
    ]),
  );
