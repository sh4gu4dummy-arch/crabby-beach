...
Green crab: Imagine stills per brush. Paint can swaps the whole image.
```

## export-naming.md

```md
# export-naming.md — v.129

Current product version: **v.128** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.129**.
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.127",
    "file": "crabby-beach-v.127-code.md",
    "bytes": 190561,
    "mtime": "2026-09-11T09:26:32.629Z"
  },
  "codebase": {
    "version": "v.090",
...
export const APP_VERSION = "v.128";
...
async function loadAssets(): Promise<Assets> {
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) =>
      paintIds.flatMap((paint) => [
        loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-${paint}-0.png?v=127`)),
        loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-${paint}-1.png?v=127`)),
        ...[1, 2, 3, 4].map((i) =>
          loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${paint}-${i}.png?v=127`)),
        ),
      ]),
    ),
  );
...
function lookSrc(color: CrabColor, hat: CrabHat) {
  ...
  return assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=123`);
}
