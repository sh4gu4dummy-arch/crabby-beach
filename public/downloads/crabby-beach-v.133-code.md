...
Current product: see `VERSION` and `src/lib/version.ts` (now **v.133**).
...
Local git + GitHub: every version. Do not ask. Do not skip the number.
Zip/APK: only when asked. Labels = files on disk.
Publish: user clicks it.
Clock voice clips: public/game/cinema/clock-1-6.mp4 and clock-7-12.mp4 (not spliced in yet).
```

## export-naming.md

```md
# export-naming.md — v.134

Current product version: **v.133** (Crabby Beach)
...
- This naming document started at `v.001` and is now **v.134**.
...
```json
{
  ...
  "scripts": {
    ...
    "build:pages:github": "node scripts/build-pages.mjs github",
    "build:pages:cloudflare": "node scripts/build-pages.mjs cloudflare"
  },
  "dependencies": {
...
export const EXPORTS: Record<ExportKind, ExportFile | null> = {
  "codeOnly": {
    "version": "v.132",
    "file": "crabby-beach-v.132-code.md",
    "bytes": 194046,
    "mtime": "2026-09-18T05:57:44.406Z"
  },
  "codebase": {
    "version": "v.090",
...
export const APP_VERSION = "v.133";
...
type Assets = {
  ...
  skins: Record<Exclude<CrabColor, "red">, Record<PaintId, CrabFrames>>;
  looks: Record<CrabColor, Record<Exclude<CrabHat, "none">, Record<PaintId, CrabFrames>>>;
};
...
async function loadAssets(): Promise<Assets> {
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) =>
      paintIds.flatMap((paint) => [
        loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-${paint}-0.png?v=129`)),
        loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-${paint}-1.png?v=129`)),
        ...[1, 2, 3, 4].map((i) =>
          loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${paint}-${i}.png?v=129`)),
        ),
      ]),
    ),
  );
  ...
  const skins = {} as Record<Exclude<CrabColor, "red">, Record<PaintId, CrabFrames>>;
  for (const id of skinIds) {
    skins[id] = {} as Record<PaintId, CrabFrames>;
    for (const paint of paintIds) {
      const slice = rest.slice(o, o + 6) as HTMLImageElement[];
      const idle0 = slice[0]!;
      const idle1 = slice[1]!;
      skins[id][paint] = { idle: [idle0, idle1, idle0, idle1], walk: slice.slice(2, 6) };
      o += 6;
    }
  }
  const looks = {} as Record<CrabColor, Record<Exclude<CrabHat, "none">, Record<PaintId, CrabFrames>>>;
  for (const color of lookColors) {
    looks[color] = {} as Record<Exclude<CrabHat, "none">, Record<PaintId, CrabFrames>>;
    for (const hat of lookHats) {
      looks[color][hat] = {} as Record<PaintId, CrabFrames>;
      for (const paint of paintIds) {
        const slice = rest.slice(o, o + 6) as HTMLImageElement[];
        const idle0 = slice[0]!;
        const idle1 = slice[1]!;
        looks[color][hat][paint] = { idle: [idle0, idle1, idle0, idle1], walk: slice.slice(2, 6) };
        o += 6;
      }
    }
  }
...
export function createGame(
  ...
  function crabFrame(): CanvasImageSource {
    ...
    const kit =
      hat === "none"
        ? body === "red"
          ? (a.crab[paint] ?? a.crab.green)
          : a.skins[body][paint]
        : a.looks[body][hat][paint];
  ...
  return (
    <div className="flex h-full min-h-[100vh] w-full justify-center overflow-hidden bg-sand" style={inGame ? { background: hud.skyFill } : nightBg}>
      ...
      {showCartoon && (
        <CartoonOverlay
          src={assetUrl(
            showCartoon.startsWith("s01e04")
              ? "game/cinema/s01e04.mp4?v=125"
              : showCartoon.startsWith("s01e03")
                ? "game/cinema/s01e03.mp4?v=125"
    ...
    <div className="absolute inset-0 z-30 flex flex-col overflow-y-auto bg-[#120c18] text-[#fff6e8]">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-sm flex-1 flex-col px-4 pt-[max(1.1rem,env(safe-area-inset-top))] pb-[max(1.1rem,env(safe-area-inset-bottom))]">
        ...
        <div className="mt-5 grid gap-4">
          ...
          <button
            type="button"
            onClick={() => onCartoon("s01e03")}
            className="overflow-hidden rounded-[1.4rem] bg-[#1c1422] text-left shadow-lg ring-2 ring-[#e8c07a]/50"
          >
            <div className="relative aspect-[16/10] overflow-hidden bg-[#2a1c28]">
              <img src={assetUrl("game/cinema/s01e03.jpg?v=125")} alt="" className="absolute inset-0 h-full w-full object-cover" />
              <span className="absolute right-3 bottom-3 grid size-11 place-items-center rounded-full bg-coral text-cream shadow-md">
                <Play className="size-5" fill="currentColor" />
              </span>
            </div>
            <div className="px-4 py-3">
              <p className="text-xs font-semibold tracking-wide text-[#e8c07a] uppercase">Ep 1.3</p>
              <p className="text-lg font-bold">Red meets Yellow</p>
            </div>
          </button>
          <button
            type="button"
            onClick={() => onCartoon("s01e04")}
            className="overflow-hidden rounded-[1.4rem] bg-[#1c1422] text-left shadow-lg ring-2 ring-[#e8c07a]/50"
          >
            <div className="relative aspect-[16/10] overflow-hidden bg-[#2a1c28]">
              <img src={assetUrl("game/cinema/s01e04.jpg?v=125")} alt="" className="absolute inset-0 h-full w-full object-cover" />
              <span className="absolute right-3 bottom-3 grid size-11 place-items-center rounded-full bg-coral text-cream shadow-md">
                <Play className="size-5" fill="currentColor" />
              </span>
            </div>
            <div className="px-4 py-3">
              <p className="text-xs font-semibold tracking-wide text-[#e8c07a] uppercase">Ep 1.4</p>
              <p className="text-lg font-bold">Red meets Blue</p>
            </div>
          </button>
          <button
            type="button"
...
function lookSrc(color: CrabColor, hat: CrabHat) {
  ...
  return assetUrl(`game/crabby/looks/${color}/${hat}/idle-0.png?v=129`);
}
