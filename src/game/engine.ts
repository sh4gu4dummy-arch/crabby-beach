...
async function loadAssets(): Promise<Assets> {
  ...
  const lookFiles = lookColors.flatMap((color) =>
    lookHats.flatMap((hat) =>
      paintIds.flatMap((paint) => [
        loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-${paint}-0.png?v=136`)),
        loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/idle-${paint}-1.png?v=136`)),
        ...[1, 2, 3, 4].map((i) =>
          loadImage(assetUrl(`game/crabby/looks/${color}/${hat}/walk-${paint}-${i}.png?v=136`)),
        ),
      ]),
    ),
  );
